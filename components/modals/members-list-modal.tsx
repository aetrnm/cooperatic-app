...
const roleIconMap = {
  USER: null,
  MODERATOR: <StarHalf className="h-4 w-4 text-blue-500" />,
  ADMIN: <Star className="h-4 w-4 text-red-500" />,
};
...
export const MembersListModal = () => {
  ...
  const isModalOpen = isOpen && type === "membersList";
  ...
  const onKick = async (memberId: string) => {
    try {
      ...
      router.refresh();
      onOpen("membersList", { group: response.data });
    } catch (error) {
      console.log(error);
    } finally {
      setLoadingId("");
        ...
        <ScrollArea className="mt-8 max-h-[420px] pr-6">
          {group?.members?.map((member) => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              {group.profileId !== member.profileId &&
                loadingId !== member.id && (
                        ...
                        <DropdownMenuSub>
                          ...
                          <DropdownMenuPortal>
                            <DropdownMenuSubContent>
                              ...
                              >
                                <StarHalf className="h-4 w-4 mr-2 text-blue-500" />
                                Moderator
